...
class QueryRouter:
    def __init__(self, conn):
        ...
        self.patterns: List[Tuple[str, str]] = [
            ...
            (r"(?:(?:a\s+)?(?:qu[eé]|cu[aá]l)\s+(?:era\s+)?(?:(?:la|el)\s+)?(?:profesión|oficio|ocupación|(?:actividad|empleo)\s+(?:laboral|profesional)?|tipo\s+de\s+(?:trabajo|actividad|empleo)|medio\s+de\s+vida)(?:\s+(?:ten[ií]a|figura|aparece|se\s+dedic[oó]|consta|desempe[nñ][oó]|realiz[oó]|era|fue))?\s+.+|de\s+qu[eé]\s+trabaja(?:ba)?\s+.+|en\s+qu[eé]\s+trabaja(?:ba)?\s+.+|con\s+qu[eé]\s+(?:oficio|trabajo|actividad|empleo)\s+(?:aparece|figura|documentado)\s+.+|(?:hay\s+)?constancia\s+(?:de|del)\s+(?:empleo|oficio|trabajo|actividad)\s+(?:de|para)\s+.+|c[oó]mo\s+se\s+ganaba\s+la\s+vida\s+.+)", "handle_occupation_natural"),
