...
def _noun(noun):
    """Traduce un nombre de parentesco (p.ej. 'tíos maternos', 'madre política o
    suegra') al idioma actual vía la clave 'kin_nouns.<es>'. Fallback: el propio
    texto español, para que nunca quede una clave sin resolver."""
    key = "kin_nouns." + noun
    out = _t(key)
...
class QueryRouter:
    def __init__(self, conn):
        ...
        self.patterns: List[Tuple[str, str]] = [
            ...
            (r"(?:qu[eé]\s+hijos\s+(?:documentados?\s+)?tuvo\s+.+|what\s+children\s+did\s+.+\s+have|(?:qu[eé]\s+)?descendencia\s+directa\s+(?:tuvo|tiene|ten[ií]a)\s+.+)", "handle_children"),
            ...
            (r"(?:tatarabuelos\s+(?:de|constan\s+para)\s+.+)", "handle_great_great_grandparents"),
    ...
    def _extract_subject_name(self, question: str) -> Optional[str]:
        patterns = [
            ...
            r"descendencia\s+directa\s+(?:tuvo|tiene|ten[ií]a)\s+(.+?)(?:\?|$)",
            r"hijos\s+de\s+(.+?)(?:\?|$)",
            r"fills\s+de\s+(.+?)(?:\?|$)",
            ...
            r"tatarabuelos\s+constan\s+para\s+(.+?)(?:\?|$)",
    ...
    def handle_family_stats(self, question):
        ...
        rows_out = [
            _t("handle_family_stats.6", a=marriages)
            + (_t("handle_family_stats.7", a=partnerships) if partnerships else ""),
            _t("handle_family_stats.8", a=living, b=deceased),
        ]
        if min_y and max_y:
    ...
    def handle_birth_date_search(self, question):
        ...
        if not rows:
            ...
            r=rows[0]
            place=_t("frag.at_place", a=r['birth_place']) if r.get('birth_place') else ''
    ...
    def handle_death_date_search(self, question):
        ...
        if not rows:
            ...
            r=rows[0]
            place=_t("frag.at_place", a=r['death_place']) if r.get('death_place') else ''
