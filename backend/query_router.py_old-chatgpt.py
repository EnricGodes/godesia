...
class QueryRouter:
    def __init__(self, conn):
        ...
        self.patterns: List[Tuple[str, str]] = [
            ...
            (r"(?:qu[eé]\s+edad\s+ten[ií]a\s+.+\s+cuando\s+se\s+cas[oó])", "handle_age_at_marriage"),
            ...
            (r"(?:qu[ií]enes\s+eran\s+los\s+abuelos\s+de\s+.+\s+y\s+en\s+qu[eé]\s+a[nñ]os\s+nacieron)", "handle_grandparents_with_years"),
            (r"(?:c[oó]mo\s+se\s+llamaban\s+los\s+abuelos\s+de|qu[ií]enes\s+eran\s+los\s+abuelos\s+de)", "handle_grandparents_names"),
            (r"(?:en\s+qu[eé]\s+posici[oó]n\s+entre\s+sus\s+hermanos\s+naci[oó]\s+.+)", "handle_birth_order_among_siblings"),
            (r"(?:qu[ií]enes\s+eran\s+los\s+padres\s+de\s+.+\s+y\s+d[oó]nde\s+se\s+casaron)", "handle_parents_and_marriage_place"),
    ...
    def _list_people_answer(self, prefix: str, people, limit: int = 25):
        ...
        if not people:
            return prefix + " No he encontrado resultados."
        names = ", ".join(p["name"] for p in people[:limit])
    ...
    def handle_age_at_first_child(self, question):
        q = _clean_question(question)
        m = re.search(r"cu[aá]ntos?\s+a[nñ]os\s+ten[ií]a\s+(.+?)\s+cuando\s+naci[oó]\s+su\s+primer\s+hij[oa](?:,\s+(.+?))?(?:\?|$)", q, re.I)
        if not m:
            return None
    ...
    def handle_age_at_marriage(self, question):
        q = _clean_question(question)
        m = re.search(r"qu[eé]\s+edad\s+ten[ií]a\s+(.+?)\s+cuando\s+se\s+cas[oó](?:\?|$)", q, re.I)
        if not m:
            return None
        person, _ = self._resolve_person(m.group(1))
        if not person:
    ...
    def handle_grandparents_names(self, question):
        q = _clean_question(question)
        m = re.search(r"(?:c[oó]mo\s+se\s+llamaban\s+los\s+abuelos\s+de|qu[ií]enes\s+eran\s+los\s+abuelos\s+de)\s+(.+?)(?:\?|$)", q, re.I)
        if not m:
            return None
        person, _ = self._resolve_person(m.group(1))
        if not person:
