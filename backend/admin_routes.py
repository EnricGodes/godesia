...
def _photos_target_dir(subdir: str) -> Path:
    d = _base_dir / "data" / "photos"
    if subdir:
        if not _SAFE_SUBDIR.match(subdir):
            raise HTTPException(status_code=400, detail="subdir inválido")
        d = d / subdir
    return d
...
async def upload_photos(files: list[UploadFile] = File(...), subdir: str = Form("")):
    """Bulk upload photos to data/photos/ (o a una subcarpeta whitelisted via `subdir`)."""
    photos_dir = _photos_target_dir(subdir)
